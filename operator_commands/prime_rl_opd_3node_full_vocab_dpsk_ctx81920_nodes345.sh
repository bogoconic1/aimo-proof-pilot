...
case "${PRIME_COMPONENT_ROLE}" in
  policy_inference)
    ...
    exec "${TRAIN_PY_ENV[@]}" /usr/bin/python /app/train.py "${COMMON_ARGS[@]}" \
      ...
      --prime_vllm_extra "${PRIME_VLLM_EXTRA:-{\"kv_cache_dtype\":\"fp8\",\"block_size\":256,\"disable_custom_all_reduce\":true,\"compilation_config\":{\"cudagraph_mode\":\"NONE\",\"max_cudagraph_capture_size\":0,\"pass_config\":{\"fuse_allreduce_rms\":false}}}}"
    ;;
  ...
  teacher_inference)
    ...
    exec "${TRAIN_PY_ENV[@]}" /usr/bin/python /app/train.py "${COMMON_ARGS[@]}" \
      ...
      --prime_opd_teacher_vllm_extra "${PRIME_OPD_TEACHER_VLLM_EXTRA:-{\"kv_cache_dtype\":\"fp8\",\"block_size\":256,\"enable_expert_parallel\":true,\"linear_backend\":\"deep_gemm\",\"disable_custom_all_reduce\":true,\"compilation_config\":{\"cudagraph_mode\":\"NONE\",\"max_cudagraph_capture_size\":0,\"pass_config\":{\"fuse_allreduce_rms\":false}}}}"
    ;;
